...
from PyPDF2 import PdfReader  # Add PyPDF2 for PDF text extraction
from summarizer import Summarizer  # Install bert-extractive-summarizer

start = time.time()

# Load NLP model and BERT model
nlp = spacy.load("en_core_web_sm")

# Configure the BERT model to output hidden states
config = BertConfig.from_pretrained("bert-base-uncased", output_hidden_states=True)
bert_model = AutoModel.from_pretrained("bert-base-uncased", config=config)

# Initialize the summarizer with the updated BERT model
summarizer = Summarizer(custom_model=bert_model, hidden=[-2])  # Use the second-to-last layer
...
# Summarize text with BERT
def summarize_text_with_bert(text):
    return summarizer(text, min_length=60, max_length=150, ratio=0.2)

# Summarize policy text in chunks
def summarize_policy_bert(cleaned_text):
    """Summarize the cleaned text using BERT in chunks."""
    chunks = split_text(cleaned_text)
    summarized_text = " ".join([summarize_text_with_bert(chunk) for chunk in chunks])
    return summarized_text

def format_bert_summary(summary):
    """Format BERT summary into bullet points and make it concise."""
...
def extract_permissions_and_reasons(cleaned_text):
    ...
    # Check for common permission-related phrases based on keywords
    ...
    # If permissions are found, remove duplicates
    unique_permissions = []
...
def format_permissions(permissions_with_reasons):
    ...
    output = ["Permissions/Allowances and Reasons:\n"]
    for permission, reason in permissions_with_reasons:
...
def main():
    ...
    policy_summary = summarize_policy_bert(cleaned_text)
    formatted_summary = format_bert_summary(policy_summary)
    print("\nBERT Summary:\n", formatted_summary)
